...
public class BookManagerControllerTests
{
    ...
    public void DeleteBookById_Deletes_Correct_Book()
    {
        ...
        var existingBookFound = GetTestBooks().FirstOrDefault(b => b.Id.Equals(existingBookId));

        _mockBookManagementService.Setup(b => b.DeleteById(existingBookId)).Returns(existingBookFound);
        //Act
        var result = _controller.DeleteById(existingBookId);
